...
def auto_verificar_apuestas():
    ...
    # 1. API LIGA MX (ESPN) CON PARSEO DE PERÍODOS (1HT Y FULL TIME)
        ...
        if r_mx.status_code == 200:
            ...
            for ev_item in events:
                ...
                for t in teams:
                    ...
                    linescore = t.get("linescores", [])
                    t_1ht = int(linescore[0].get("value", 0)) if len(linescore) >= 1 else 0
                    
                    if is_home:
                        loc_name, loc_score, loc_1ht = t_name, t_score, t_1ht
                    else:
                        vis_name, vis_score, vis_1ht = t_name, t_score, t_1ht
            ...
            if "Liga MX" in dep:
                for match_title, score_data in res_mx.items():
                    if item["equipo_loc"].lower() in match_title.lower() and item["equipo_vis"].lower() in match_title.lower():
                        ...
                        g_1ht = score_data["g_1ht"]
                        tot_goles = g_loc + g_vis
                        mercado = item["mercado"]
                        linea_str = item.get("linea", "")
                        
                        # A) MERCADOS PRIMERA MITAD (1HT) - EVALUACIÓN EN VIVO / MEDIO TIEMPO
                        if "1ra Mitad" in mercado or "1HT" in mercado:
                            try:
                                val_linea = float(linea_str) if linea_str.replace('.','',1).isdigit() else 0.5
                            except Exception:
                                val_linea = 0.5

                            if g_1ht > val_linea:
                                item["estado"] = "WIN"
                                item["resultado_real"] = f"{g_1ht} Goles (1HT)"
                                actualizados += 1
                            elif score_data["period"] >= 2 or score_data["completed"]:
                                item["estado"] = "LOSS"
                                item["resultado_real"] = f"{g_1ht} Goles (1HT Final)"
                                actualizados += 1

                        # B) MERCADOS DE RESULTADO FINAL / 90 MINUTOS
                            ...
                            elif "Más de 2.5" in mercado or "2.5" in linea_str:
...
def render_tabla_historial_html(data_list):
    if not data_list:
        st.caption("No hay registros en esta sección.")
        return
    rows_html = ""
...
if not es_mlb:
    ...
    with col_izq:
        ...
        <div class="header-big-left">
            <img src="https://a.espncdn.com/combiner/i?img=/i/leaguelogos/soccer/500/22.png" style="height: 55px; margin-right: 15px; filter: drop-shadow(0 0 6px #f5d742);">
        ...
        with c_esc1:
            ...
            <div class="team-badge-card">
                ...
                <div>
                    ...
                    <div style="color: #00ff66; font-weight: 800; font-size: 14px;">{xg_local:.2f} <span style="font-size: 11px; color: #f5d742;">xG Esperado</span></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with c_esc2:
            ...
            <div class="team-badge-card">
                ...
                <div>
                    ...
                    <div style="color: #f5d742; font-weight: 800; font-size: 14px;">{xg_visita:.2f} <span style="font-size: 11px; color: #f5d742;">xG Esperado</span></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        fig_pie = go.Figure(data=[go.Pie(
            ...
            marker=dict(colors=['#00ff66', '#f59e0b', '#f5d742'])
        )])
        ...
        with st.expander("⚙️ MOMIOS DE TU CASA DE APUESTAS (CALCULADORA EV)", expanded=True):
            ...
            st.markdown("<p style='color:#f5d742; font-weight:800; margin-top:8px;'>PROPS DE CÓRNERS (7 A 11 ENTEROS EXACTOS)</p>", unsafe_allow_html=True)
    ...
    with col_der:
        ...
        <div class="header-big-right">
            <span style="font-size: 32px; margin-right: 10px; filter: drop-shadow(0 0 6px #f5d742);">👑</span>
