...
# ------------------------------------------------------------------------------
# PERSISTENCIA EN DISCO (JSON LOCAL)
# ------------------------------------------------------------------------------
HISTORIAL_FILE = "apuestas_historial.json"


def cargar_historial_disco():
...
def guardar_historial_disco(datos):
  try:
...
def simular_montecarlo_avanzado(
    d_loc,
    d_vis,
    fatiga_loc,
    rot_loc,
  ...
  tarjetas_totales = np.random.poisson(
      (d_loc["tarjetas"] + d_vis["tarjetas"]), n_sim
  )
...
def simular_montecarlo_nfl(
  ...
  factor_clima = (
      1.0 - (0.15 if clima_viento else 0.0) - (0.10 if clima_frio else 0.0)
  )
  exp_td_loc = (
      d_loc.get("td_exp", 3.0) * (0.75 if baja_qb_loc else 1.0) * factor_clima
  )
  exp_td_vis = (
      d_vis.get("td_exp", 2.8) * (0.75 if baja_qb_vis else 1.0) * factor_clima
  )
...
def simular_montecarlo_mlb(
  ...
  env_factor = (
      d_loc["park_factor"]
      * (1.08 if viento_out else 1.0)
      * (0.95 if humedad_alta else 1.0)
  )
  ...
  exp_f5_loc = max(
      2.2 * (off_loc / (d_vis["sp_xera"] / 3.80)) * (env_factor / 1.0), 0.3
  )
  exp_f5_vis = max(
      1.9 * (off_vis / (d_loc["sp_xera"] / 3.80)) * (env_factor / 1.0), 0.3
  )
  ...
  exp_k_loc = max(
      5.5 * (d_loc["sp_k_pct"] / 0.23) * (1.1 if not bvp_favor_vis else 0.9), 1.0
  )
  exp_k_vis = max(
      5.2 * (d_vis["sp_k_pct"] / 0.23) * (1.1 if not bvp_favor_loc else 0.9), 1.0
  )
...
def generar_grafica_mini_15_partidos(prob_exito):
  ...
  fig = go.Figure()
  fig.add_trace(
      go.Bar(x=labels, y=[1] * 15, marker_color=colors, hoverinfo="x")
  )
  fig.update_layout(
...
def generar_grafica_efectividad_capsulas_3d(list_apuestas):
  ...
  wins = [
      sum(
          1
          for a in list_apuestas
          if a["liga"] == l and a["resultado"] == "WIN"
      )
      for l in ligas
  ]
  looses = [
      sum(
          1
          for a in list_apuestas
          if a["liga"] == l and a["resultado"] == "LOOSE"
      )
      for l in ligas
  ]
  ...
  fig.update_layout(
      ...
      legend=dict(
          ...
          xanchor="right",
          x=1,
          font=dict(size=11, color="#0f172a", family="Syne"),
      ),
      xaxis=dict(
          showgrid=False, tickfont=dict(size=11, color="#0f172a", family="Syne")
      ),
      yaxis=dict(
          showgrid=True, gridcolor="#e2e8f0", tickfont=dict(size=10, color="#64748b")
      ),
  )
  return fig
...
# ------------------------------------------------------------------------------
# SESSION STATE & TRACKER PERSISTENTE GLOBAL (CONEXIÓN A DISCO)
# ------------------------------------------------------------------------------
OPCIONES_ESTADO = ["⏳ PENDIENTE", "WIN", "LOOSE"]

if "apuestas_registradas" not in st.session_state:
  st.session_state["apuestas_registradas"] = cargar_historial_disco()
...
def guardar_apuesta_seleccionada(liga, partido, mercado, cuota):
  nuevo_id = (
      max(
          [a["id"] for a in st.session_state["apuestas_registradas"]], default=0
      )
      + 1
  )
  nueva_apuesta = {
  ...
  st.session_state["apuestas_registradas"].append(nueva_apuesta)
  guardar_historial_disco(st.session_state["apuestas_registradas"])
...
MLB_MOMIOS_REALES_HOY = {
    ...
    "Cincinnati Reds": 2.80,
    "New York Yankees": 1.72,
    "Minnesota Twins": 2.15,
    "Chicago Cubs": 2.05,
    "Atlanta Braves": 1.80,
    "Cleveland Guardians": 1.52,
    "Chicago White Sox": 2.60,
    "New York Mets": 1.85,
    "Baltimore Orioles": 1.95,
}
...
BUNDESLIGA_DATA = {
    "Friburgo": {
        "logo": "https://crests.football-data.org/160.png",
        "xg_loc": 1.55,
        "xga_loc": 1.25,
        "xg_vis": 1.30,
        "xga_vis": 1.45,
    ...
    "SV Elversberg": {
        "logo": "https://crests.football-data.org/6706.png",
        "xg_loc": 1.20,
        "xga_loc": 1.60,
        "xg_vis": 0.95,
        "xga_vis": 1.80,
    ...
    "Schalke 04": {
        "logo": "https://crests.football-data.org/6.png",
        "xg_loc": 1.30,
        "xga_loc": 1.55,
        "xg_vis": 1.05,
        "xga_vis": 1.75,
    ...
    "Hamburg": {
        "logo": "https://crests.football-data.org/26.png",
        "xg_loc": 1.35,
        "xga_loc": 1.50,
        "xg_vis": 1.10,
        "xga_vis": 1.70,
...
CHAMPIONS_DATA = {
    ...
    "AEK": {
        "logo": "https://crests.football-data.org/1075.png",
        "xg_loc": 1.30,
        "xga_loc": 1.40,
        "xg_vis": 1.10,
        "xga_vis": 1.60,
    ...
    "Fenerbahçe": {
        "logo": "https://crests.football-data.org/613.png",
        "xg_loc": 1.65,
        "xga_loc": 1.25,
        "xg_vis": 1.40,
        "xga_vis": 1.45,
    ...
    "Slovan Bratislava": {
        "logo": "https://crests.football-data.org/1816.png",
        "xg_loc": 1.10,
        "xga_loc": 1.85,
        "xg_vis": 0.90,
        "xga_vis": 2.10,
    ...
    "Shakhtar": {
        "logo": "https://crests.football-data.org/588.png",
        "xg_loc": 1.40,
        "xga_loc": 1.50,
        "xg_vis": 1.20,
        "xga_vis": 1.70,
    ...
    "Como": {
        "logo": "https://crests.football-data.org/1072.png",
        "xg_loc": 1.25,
        "xga_loc": 1.55,
        "xg_vis": 1.05,
        "xga_vis": 1.75,
    ...
    "Bodø/Glimt": {
        "logo": "https://crests.football-data.org/1149.png",
        "xg_loc": 1.30,
        "xga_loc": 1.60,
        "xg_vis": 1.10,
        "xga_vis": 1.80,
    ...
    "Sabah Futbol": {
        "logo": "https://crests.football-data.org/8157.png",
        "xg_loc": 1.05,
        "xga_loc": 1.90,
        "xg_vis": 0.85,
        "xga_vis": 2.20,
    ...
    "Viking": {
        "logo": "https://crests.football-data.org/1148.png",
        "xg_loc": 1.20,
        "xga_loc": 1.65,
        "xg_vis": 1.00,
        "xga_vis": 1.85,
    ...
    "Slavia Praga": {
        "logo": "https://crests.football-data.org/583.png",
        "xg_loc": 1.35,
        "xga_loc": 1.30,
        "xg_vis": 1.15,
        "xga_vis": 1.50,
    ...
    "Manchester United": {
        ...
        "xg_loc": 1.60, "xga_loc": 1.45, "xg_vis": 1.35, "xga_vis": 1.55, "ppda": 10.8, "aereos": 50, "corners": 5.9, "tarjetas": 2.2},
